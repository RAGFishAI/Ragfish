        ...
        $('#crop-button').click(function () {
            ...
            if (in_val == 11) {
                cropper.croppie('result', { type: 'base64', size: 'original', quality: 0.5, format: 'jpeg', }).then(function (dataUrl) {
                    ...
                    $("#listingimagename").text(filename)
                    $('#listingimage').attr('src', dataUrl)
                    $('#listingimage').addClass("w-full h-full object-contain")
                    $('.CropperSection').addClass('hidden')
                    $('.mainsection').show()
                    $("#imageRemoveDiv").show()
                    $("#imageUploadDiv").hide()
    ...
    if (Cookie != '') {
        for (let key in languagedata.Toast) {
            ...
            if (key == content) {
                ...
                notify_content = `<div class="absolute z-[50] top-[32px] right-[32px] max-w-[358px] w-full flex flex-col gap-[16px]">
                   <div class="toast-msg bg-[#F4FFF8] rounded-[8px] shadow-[0px_3px_10px_0px_#00000029] relative p-[16px] flex justify-start items-start gap-[10px] border-1 border-[#E3E3E3] border-l-[3px] border-l-[#156332]">
            <span> <img src="/public/img/success.svg" alt=""> </span>
            <div class="">
                <h3 class="text-[#202732] text-[14px] font-medium">Success</h3>
                <p class="text-[#3C485E] text-[13px] font-normal my-[4px_1px] leading-[14px]">` + languagedata.Toast[key] + `</p>
        ...
        for (let key in languagedata.Toast) {
            if (key == alertMsg) {
                console.log(msg, "msg");
                notify_content = `<div class="absolute  z-[50] top-[32px] right-[32px] max-w-[358px] w-full flex flex-col gap-[16px]"><div class="toast-msg bg-[#FFE8E9] rounded-[8px] shadow-[0px_3px_10px_0px_#00000029] relative p-[16px] flex justify-start items-start gap-[10px] border-1 border-[#E3E3E3] border-l-[3px] border-l-[#991F24]">
...
$(document).ready(function () {
    ...
    $("#copyBtn").click(function (e) {
        ...
        navigator.clipboard.writeText(input.value).then(() => {
            ...
            const btnText = $(this).find(".copy_text");
            const original = btnText.text();

            btnText.text("Copied!");
            setTimeout(() => btnText.text(original), 1500);

            // --- Insert the styled toast UI instead of showToast() ---
            let notify_content = `
            <div class="absolute top-[32px]  z-[50] right-[32px] max-w-[358px] w-full flex flex-col gap-[16px]">
                <div class="toast-msg bg-[#F4FFF8] rounded-[8px] shadow-[0px_3px_10px_0px_#00000029] relative p-[16px] flex justify-start items-start gap-[10px] border-1 border-[#E3E3E3] border-l-[3px] border-l-[#156332]">
                    <span><img src="/public/img/success.svg" alt=""></span>
                    <div>
                        <h3 class="text-[#202732] text-[14px] font-medium">Success</h3>
                        <p class="text-[#3C485E] text-[13px] font-normal my-[4px_1px] leading-[14px]">Link Copied Successfully</p>
                    </div>
                    <a href="javascript:void(0)" class="absolute right-[16px] top-[16px] w-[16px] h-[16px] flex items-center justify-center">
                        <img src="/public/img/close.svg" alt="" class="w-[10px] cancel-notify">
                    </a>
                </div>
            </div>
            `;
            ...
            // Allow manual close
            $(document).on('click', '.cancel-notify', function () {
                $(this).closest('.toast-msg').fadeOut('fast', function () {
                    $(this).remove();
                });
            });
            // --------------------------------------------------------
    ...
    $('#urlinput, #copyBtn').on('click mousedown', function(e) {
        e.stopPropagation();
    });
